...
双击 **`Stop-EnglishLehrer.cmd`** 可关闭后台服务：先保存正在进行的课堂为中断状态、释放语音连接，再退出。脚本会核对服务属于当前项目目录；已关闭时重复执行也无妨。浏览器标签页可随后自行关闭。只关浏览器不会停止后台服务。手动运行时也可按 `Ctrl+C` 关闭。

## 已实现功能

- **可执行教案**：在 **Vorbereitung** 选择主题，再点 **Unterrichtsplan & Lernbelege → Plan erstellen**。AI 根据主题、已教内容和到期复习词生成具体步骤；可预览白板、上下移动步骤、修改练习方式、德语含义和预计时间，再点 **Plan speichern**。在此页面点击 **Mikrofon an & Stunde starten**，即可用当前主题直接开始课程。时间仅供备课参考，绝不作为孩子的答题倒计时。调整后若把未教单词的测验放到示范之前，会提示修正并保留旧计划。
- **教学素材**：优先使用本地 emoji，找不到语义匹配的 emoji 时直接显示德语文字，将看图说英语改为德语→英语口说。不再调用 GPT Image，也没有图片等待或重试步骤。预览与预取不会产生学习记录。
...
- 本地 **3,720 个 Twemoji SVG 素材**，支持英文/德文名称查询；bus、train、sofa 等显示大 emoji，基础色球仍精准绘制。无匹配 emoji 时显示德语文字，不使用不相关图标或色块替代。
...
- 约 9 分钟提醒模型收尾。听到孩子说 Tschüss / Tschüsschen / Tüssche 等告别后，3 秒没有新输入就主动结束并停止麦克风。新字幕、再次开口或点击答题取消本次倒计时；单纯讨论“如何说 Tschüss”不会触发。普通停顿没有超时。模型自行收尾仍等待实际发言完成；随时可以点结束。
...
- **单节学习结果删除**：在 **Mein Lernweg → Deine bisherigen Stunden** 点某节课右侧垃圾桶，或打开结果页点 **Diese Lernergebnisse löschen**。确认后删除该课的词句、题目、回答和总结，重新计算掌握度、复习与统计；其他课程及主题保留。正在进行的课须先结束。已有备课聊天文字保留，需要时可另点 Gespräch leeren。
...
Thinking effort 通过 `ENGLISH_TEACHER_REASONING_EFFORT` 配置，默认保持原来的 `low`；可按所选模型支持范围设置 `none`、`minimal`、`low`、`medium`、`high`、`xhigh` 或 `max`。这对应 Responses API 的 `reasoning.effort`，可用档位因模型而异；Live 语音和音频转写不发送这个参数。参见 [OpenAI 推理模型文档](https://developers.openai.com/api/docs/guides/reasoning)。修改后重启应用。
...
设置页展示当前实际加载的模型与密钥配置状态，不显示密钥内容。新模型必须支持对应接口：语音使用 Live，后台使用 Responses 和函数工具。不同接口体系（例如旧 Realtime）不能只改模型名直接替换。
...
完整口头答题测试：先运行 `powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts/make-test-speech.ps1` 生成合成德语语音（需要 Windows 德语语音包），再运行 `node scripts/smoke-live.js --conversation`。它验证真实语音请求出题、点击答错、口头答对和 SQLite 保存。不会使用或录制真实麦克风。上述真实 API 测试不能替代儿童实际说话时的听感和教学质量验收。
